...
with open('automato3.txt', 'r') as f:
...
for x in t:
    if x[1] not in simbolos:
        simbolos.append(x[1])
aceitacao = list(map(int, lines[1]))   # transformando estados de aceitação em inteiros

# pegando as entradas para testes
with open('automato3_entrada.txt', 'r') as f:
...
print("\nResultado registrado em 'automato3_resultado.txt'")
input()
